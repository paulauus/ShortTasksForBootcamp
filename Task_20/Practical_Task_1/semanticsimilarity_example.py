...
'''nlp = spacy.load('en_core_web_sm')''' # For comparing similarity scores between medium and small language models.
...
# With .md model the similarities are higher in the same category of text (complaints and recipes), and visibly lower when complaints are compared to recipes.
        
# When using the smaller language model, we get a warning before each output:
...
# the tagger, parser and NER, which may not give useful similarity judgements. This may happen if you're using one of the small
# models, e.g. `en_core_web_sm`, which don't ship with word vectors and only use context-sensitive tensors. You can always add your
# own word vectors, or use one of the larger models instead if available.
# The similarity scores are very different (mostly smaller) with the small model.
